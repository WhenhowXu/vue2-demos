<template>
  <LayoutPage>
    <template #main>
      <vxe-toolbar ref="toolbarRef" custom :refresh="{ query: fetchDemos }"></vxe-toolbar>
      <vxe-table ref="tableRef" border :loading="loading" :data="vxeDemos">
        <vxe-column type="seq" title="序号" width="60" fixed="left"></vxe-column>
        <vxe-column field="name" title="示例名称"></vxe-column>
        <vxe-column field="describe" title="详细描述"></vxe-column>
...
<script>
import { getVxeDemos } from "@/api/table";
...
export default {
  ...
  methods: {
    async fetchDemos() {
      try {
        this.loading = true;
        this.vxeDemos = await getVxeDemos();
      } finally {
        this.loading = false;
      }
    },
